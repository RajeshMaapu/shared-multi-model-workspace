...
public actor CollaborationService {
    ...
    public func authenticate(token: String) throws -> Principal {
        ...
        if let row = try repo.db.query("SELECT engineer FROM writer_generations WHERE token_hash=? AND state IN ('writing','discussion')", [.text(hash)]).first,
           let name = row["engineer"]?.text, let engineer = EngineerID(rawValue: name) {
    ...
    public func authorizeWriterRequest(token: String?, method: String, params: JSONValue?) throws {
        guard let token else { return }
        let hash = SHA256.hash(data: Data(token.utf8)).map { String(format: "%02x", $0) }.joined()
        guard let row = try repo.db.query("SELECT task_id,state FROM writer_generations WHERE token_hash=?", [.text(hash)]).first else { return }
        guard ["writing", "discussion"].contains(row["state"]?.text ?? "") else {
            throw WorkshopError.invalidRequest("Writer capability expired")
        }
        if method == "workshop_get_capacity" { return }
        var taskID = params?["task_id"]?.stringValue
        if taskID == nil, let id = params?["subtask_id"]?.stringValue {
            taskID = try repo.subtask(SubtaskID(id))?.taskID.rawValue
        }
        guard taskID == row["task_id"]?.text else {
            throw WorkshopError.invalidRequest("Writer capability is scoped to its task")
        }
    }

    public func promoteWriterSnapshot(id: String, verifiedDigest: String, principal: Principal) throws {
        guard principal == .user, let homeDir else {
            throw WorkshopError.invalidRequest("Only the authenticated user may approve a verified snapshot")
        }
        guard let row = try repo.db.query("SELECT * FROM writer_generations WHERE id=?", [.text(id)]).first,
              let task = row["task_id"]?.text, let owner = row["engineer"]?.text,
              let engineer = EngineerID(rawValue: owner), let path = row["path"]?.text,
              let base = row["base_path"]?.text, let snapshot = row["snapshot_path"]?.text,
              let digest = row["digest"]?.text else {
            throw WorkshopError.invalidRequest("Snapshot unavailable")
        }
        let lease = WriterGenerations.Lease(id: id, taskID: TaskID(task), engineer: engineer, path: path, basePath: base)
        try WriterGenerations(db: repo.db, home: homeDir).promote(
    ...
    public func createTask(_ request: CreateTaskRequest,
        ...
        try checkStorage()
        guard request.schemaVersion == 1 || request.schemaVersion == 2 else {
            throw WorkshopError.invalidRequest(
                "unsupported schema_version: \(request.schemaVersion)")
        }
        guard !request.idempotencyKey.trimmingCharacters(in: .whitespacesAndNewlines)
            .isEmpty else {
            throw WorkshopError.invalidRequest("idempotency_key must not be empty")
        }
        var request = request
        var effectiveParticipants = request.participants
        if request.schemaVersion == 1 {
            request.idempotencyKey = Self.normalizeCodexKey(request.idempotencyKey,
                                                          principal: principal)
            guard request.collaborationMode == nil, request.origin == nil else {
                throw WorkshopError.invalidRequest(
                    "collaboration_mode and origin require schema_version 2")
            }
        } else {
            guard request.phase != .followUp else {
                throw WorkshopError.invalidRequest(
                    "Follow-ups must append to an existing task")
            }
            switch request.collaborationMode ?? .ownerOnly {
            case .ownerOnly:
                guard request.participants.allSatisfy({ $0 == .devin }) else {
                    throw WorkshopError.invalidRequest(
                        "owner_only permits only devin (or no) participants")
                }
                effectiveParticipants = [.devin]
            case .requestedPeers:
                let requested = Set(request.participants)
                effectiveParticipants = EngineerID.allCases.filter { requested.contains($0) }
                guard effectiveParticipants.contains(.kimi)
                        || effectiveParticipants.contains(.deepseek) else {
                    throw WorkshopError.invalidRequest(
                        "requested_peers requires at least one peer (kimi or deepseek)")
                }
                if !effectiveParticipants.contains(.devin) {
                    effectiveParticipants.insert(.devin, at: 0)
                }
            }
            if let origin = request.origin {
                guard principal == .codex else {
                    throw WorkshopError.invalidRequest(
                        "origin is only accepted from the codex principal")
                }
                guard !origin.sourceTaskID
                    .trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
                      !origin.invocationID
                    .trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                    throw WorkshopError.invalidRequest(
                        "origin source_task_id and invocation_id must not be empty")
                }
                guard request.idempotencyKey
                        == "codex-invocation-" + origin.invocationID else {
                    throw WorkshopError.invalidRequest(
                        "idempotency_key must be codex-invocation-<invocation_id>")
                }
            }
        }
        ...
        if let existing = try repo.operation(request.idempotencyKey) {
            if existing.principal == principal.kind && existing.payloadHash == payloadHash {
        ...
        if let origin = request.origin,
           try repo.taskIDForOrigin(principal: principal.kind,
                                    sourceTaskID: origin.sourceTaskID,
                                    invocationID: origin.invocationID) != nil {
            throw WorkshopError.idempotencyConflict
        }
        ...
        try repo.db.transaction {
            ...
            try repo.insertTaskIngress(taskID: taskID, request: request,
                                       principal: principal.kind)
            for engineer in effectiveParticipants {
            ...
            case .execution, .followUp:
                ...
                if request.schemaVersion == 2,
                   request.collaborationMode == .requestedPeers {
                    for engineer in effectiveParticipants where engineer != .devin {
                        try repo.insertWakeup(taskID: taskID, engineerID: engineer,
                                              reason: "collaboration_requested",
            ...
            let receiptState: TaskState = request.phase == .researchProposal
                ? .researching : .queued
            let r = CreateTaskReceipt(taskID: taskID, committedSeq: seq,
                                      state: receiptState,
    ...
    public func callTool(_ name: String, args: JSONValue,
        ...
        case "workshop_create_task":
            ...
            let isRunning = runningTurns
                .contains { $0.hasPrefix(receipt.taskID.rawValue + ":") }
            let hasQueuedWork = try repo.pendingOutbox(eventType: Self.dispatchRequested)
                .contains { $0.taskID == receipt.taskID }
                || !(try repo.pendingWakeups(taskID: receipt.taskID)).isEmpty
            let status: CreateTaskStatus = isRunning ? .running
                : (hasQueuedWork ? .queued : receipt.status)
            var out: [String: JSONValue] = [
                "task_id": .string(receipt.taskID.rawValue),
                "committed_seq": .number(Double(receipt.committedSeq)),
                "state": .string(current.rawValue),
    ...
    private func handleDispatch(_ row: OutboxEvent) async -> String? {
        ...
        do { ingress = try repo.taskIngress(taskID) } catch {
            blockInvalidIngress(taskID)
            return "Task ingress could not be decoded; dispatch blocked"
        }
        let schemaVersion = ingress?.request.schemaVersion ?? 1

        // Eligible engineers: participants whose probe reports available (spec §5.2).
        var order = EngineerID.allCases
        if schemaVersion == 2 {
            order = [preferredOwner ?? .devin]
        } else if let preferredOwner, let index = order.firstIndex(of: preferredOwner) {
        ...
        guard let winner, let adapter = adapters[winner] else {
            ...
            let ok = attempt("noEligibleCommit") {
                try repo.db.transaction {
                    ...
                    let body = schemaVersion == 2
                        ? "Delivery owner unavailable; waiting for Fusion or an "
                            + "explicit reassignment"
                        : "No eligible engineer available"
                    try repo.insertMessage(Message(
                        id: MessageID(newID("msg")), taskID: taskID,
                        seq: try repo.nextMessageSeq(taskID), author: .system,
    ...
    private func ensureTaskWorkspace(taskID: TaskID, ingress: TaskIngress?) throws -> TaskWorkspace? {
        ...
        if let stored {
            let expected = URL(fileURLWithPath: homeDir).standardizedFileURL
                .resolvingSymlinksInPath().appendingPathComponent("worktrees")
                .appendingPathComponent(taskID.rawValue).path
            guard stored.path == expected, FileManager.default.fileExists(atPath: stored.path) else {
                throw WorkshopError.invalidRequest("Persisted task workspace is missing or mismatched")
            }
        }
        let prepared = try WorkspaceManager.prepareTaskWorkspace(
            homeDir: homeDir, taskID: taskID, workspaceRef: ingress?.request.workspaceRef)
        if let stored {
            guard stored.path == prepared.path, stored.repositoryPath == prepared.repositoryPath,
                  stored.branch == prepared.branch else {
                throw WorkshopError.invalidRequest("Persisted task workspace identity changed")
            }
            return stored
        }
        try repo.insertTaskWorkspace(prepared)
        return prepared
    }

    private func blockInvalidIngress(_ taskID: TaskID) {
        let timestamp = now()
        _ = attempt("invalidIngress") {
            try repo.db.transaction {
                try repo.updateTaskState(taskID, .blocked, at: timestamp)
                try repo.insertMessage(Message(
                    id: MessageID(newID("msg")), taskID: taskID,
                    seq: try repo.nextMessageSeq(taskID), author: .system,
                    kind: .systemEvent,
                    body: "Task ingress could not be decoded; dispatch blocked",
                    deliveryState: .committed, createdAt: timestamp, updatedAt: timestamp))
                try repo.insertOutbox(taskID: taskID, eventType: "task.state_changed",
    ...
    private func runTurn(adapter: EngineerAdapter, engineer: EngineerID,
        ...
        if let stored: WorkshopRepository.SessionBindingRecord =
            ...
            // Old sessions may retain personal/custom instructions. Preserve
            // them, but never silently resume or summarize them into clean runs.
            guard stored.profileRevision >= 2 else {
                _ = attempt("legacyProfileBlocked") {
                    try repo.updateTaskState(task.id, .blocked, at: timestamp)
                    try repo.insertMessage(Message(
                        id: MessageID(newID("msg")), taskID: task.id,
                        seq: try repo.nextMessageSeq(task.id), author: .system,
                        kind: .systemEvent,
                        body: "Legacy instruction profile retained. Create a fresh task with an explicit brief for clean-start execution; old history was not resumed.",
                        deliveryState: .committed, createdAt: timestamp, updatedAt: timestamp))
                }
                publishCommitted()
                return
        ...
        if failedReason == nil, sawCompletion, let writerLease, let homeDir {
            do {
    ...
    private func capacityBlock(for engineer: EngineerID,
        ...
        do {
            if try repo.taskIngress(taskID)?.request.schemaVersion == 2 {
                return "Dispatch blocked by capacity; safe writer handover is not qualified, checkpoint and wait"
            }
        } catch {
            return "Task ingress could not be decoded; capacity transfer blocked"
        }
